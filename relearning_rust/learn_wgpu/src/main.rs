...
impl<'a> State<'a> {
    async fn new(window: &'a mut Window) -> Self {
        ...
        let instance_descrptor = wgpu::InstanceDescriptor {
            backends: wgpu::Backends::all(), ..Default::default()
        };
        let instance = wgpu::Instance::new(&instance_descrptor);
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        let command_encorder_descriptor = wgpu::CommandEncoderDescriptor {
            label: Some("Render Encoder"),
        };
        let mut command_encoder = self.device.create_command_encoder(&command_encorder_descriptor);
...
async fn run() {
    ...
    let (mut window, events) = glfw.create_window(
        800, 600, "Its WGPU time", glfw::WindowMode::Windowed).unwrap();
